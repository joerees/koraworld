...
const koraStrings = reactive<IKoraStrings>({
  1: {
    note: 'F',
...
export function useKoraStrings() {

  const info = (midi: number): KoraString | undefined => {
    return Object.values(koraStrings).find((str: KoraString) => str.midi === midi);
  }
